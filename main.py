...
try:
    # سيتم جلب المفتاح من إعدادات Secrets في Streamlit وليس من الكود مباشرة
    ...
    if menu == "إنشاء حساب جديد":
        ...
        if st.button("تفعيل الحساب"):
            ...
            try:
                ...
                conn.commit()
                st.success("تم الإنشاء! انتقل الآن لتسجيل الدخول")
            except:
...
if st.session_state['logged_in']:
    st.title(f"🚀 مُنجز v4.0: الشريك التقني الذكي")
    st.sidebar.success(f"متصل: {st.session_state['user']}")
    
    tool = st.sidebar.radio("الأدوات المتاحة", ["المساعد الذكي", "محول النص لصوت", "ذاكرة المشاريع"])
    
    user_input = st.chat_input("ابدأ التدريب أو اطلب كود...")
    
    if user_input:
        try:
            # ربط الهوية بالرسالة لضمان التدريب المستمر
            prompt = f"أنت مهندس برمجيات خبير. رد على {st.session_state['user']}: {user_input}"
            response = model.generate_content(prompt)
            
            st.chat_message("assistant").write(response.text)
            
            # تفعيل ميزة الصوت (TTS)
